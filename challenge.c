...
/*
    Converts dense matrix A into CSR format while computing:

        y = A * x

    @param rows         Number of rows in A
    @param cols         Number of columns in A
    @param A            Dense row-major matrix
    @param x            Input vector
    @param out_nnz      Total non-zero element count
    @param values       CSR non-zero values array
    @param col_indices  CSR column indices array
    @param row_ptrs     CSR row offsets array
